    ...
    <div className="min-h-screen bg-[#f6f8f7] text-[#142321]">
      ...
      <main className="min-h-screen lg:pl-[248px]">
        ...
        <div id="dashboard-top" className="mx-auto min-h-[calc(100vh-76px)] max-w-[1280px] overflow-y-auto px-3 pb-24 pt-3 md:px-10 md:py-5">{accessError && <div className="mb-5 rounded-xl border border-[#f3c6be] bg-[#fff0ed] px-4 py-3 text-sm font-bold text-[#c45443]">{accessError}</div>}
          <div className="mb-2 flex items-end justify-between"><div><p className="text-sm font-medium text-[#84928d] lg:hidden">{new Date().toLocaleDateString("am-ET", { dateStyle: "full" })}</p><h2 className="text-2xl font-extrabold tracking-[-0.03em] lg:hidden">እንደምን አደርክ፣ {dashboard?.user.first_name ?? "የTelegram ተጠቃሚ"} <span className="text-[#1c9961]">👋</span></h2><p className="mt-1 text-sm text-[#81908b]">{dashboard ? "ዛሬ ምን ማሳካት ትፈልጋለህ?" : "Telegram ላይ ይመዝገቡ እና ይጀምሩ"}</p></div><button onClick={openWithdrawal} className="hidden items-center gap-2 rounded-xl bg-[#183c36] px-4 py-2.5 text-sm font-bold text-white shadow-[0_6px_18px_rgba(24,60,54,.15)] transition hover:bg-[#23574d] sm:flex"><Wallet size={16} /> ገንዘብ አውጣ</button></div>
